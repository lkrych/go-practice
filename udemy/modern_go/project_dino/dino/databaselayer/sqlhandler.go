...
func (handler *SQLHandler) GetAnimalsByType(animalType string) ([]Animal, error) {
	return handler.sendQuery(fmt.Sprintf("select * from Animals where Animal_type = '%s'", animalType))
}
